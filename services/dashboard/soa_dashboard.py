#!/usr/bin/env python3
"""
HolographicMemory SOA Terminal Dashboard
...
sys.path.insert(0, str(_project_root / "services" / "holographic-memory" / "api"))
sys.path.insert(0, str(_project_root / "services" / "orchestrator"))
sys.path.insert(0, str(_project_root / "services" / "math-core"))
sys.path.insert(0, str(_project_root / "services" / "router"))
sys.path.insert(0, str(_project_root / "services" / "vault"))
sys.path.insert(0, str(_project_root / "services" / "telemetry"))
sys.path.insert(0, str(_project_root / "services" / "holographic-memory" / "core" / "native" / "holographic" / "build"))

try:
    from app_soa import app as api_app
    from orchestrator import HolographicMemoryOrchestrator
    ...
    print("Make sure to run: make dev-verify")
...
class SOADashboard:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.orchestrator = HolographicMemoryOrchestrator(state_dir=state_dir)
            
            self.console.print("✅ [green]SOA System Initialized[/green]")
            return True
        except Exception as e:
            self.console.print(f"❌ [red]Failed to initialize SOA system: {e}[/red]")
            return False
...
[bold yellow]Examples:[/bold yellow]
  ...
  benchmark --gpu
    ...
    def show_status(self):
        ...
        orchestrator_details = "SOA orchestrator running" if self.orchestrator else "Orchestrator not initialized"
        
        status_table.add_row("GPU Acceleration", gpu_status, gpu_details)
        status_table.add_row("Core Service", core_status, core_details)
        status_table.add_row("SOA Orchestrator", orchestrator_status, orchestrator_details)
        status_table.add_row("API Service", "✅ Active", "FastAPI running on port 8085")
    ...
    def show_health(self):
        ...
        components = [
            ("API Gateway", "✅ Healthy", "12ms", now),
            ("Holographic Core", "✅ Healthy", "3ms", now),
